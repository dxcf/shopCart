...
import { mapGetters } from 'vuex'
  ...
  export default {
    ...
    methods:{
      ...
      // 加入购物车
      async addShopcar(){
        //1:点击加入购物车这个按钮的时候，做的第一件事情，将参数带给服务器（发请求），通知服务器加入购物车的产品是谁
        ...
        try {
          ...
          // 一些简单的数据skuNum,通过query形式给给路由组件传递过去
          // 产品信息的数据【比较复杂：skuUnfo】,通过会话存储（不持久化，会话结束数据消失）
          // 本地存储 | 会话存储，一般存储的是字符串
